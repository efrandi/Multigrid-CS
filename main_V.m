...
%Parametri CS
delta = 1e-4; %Forse 1e-5 per fig 4
Params.red = .25; Params.aug = 1; Params.tol = 1e-5; Params.maxit = 1000;
%Parametri multigrid
Params.nu0 = 1; Params.nu1 = 3; Params.nu2 = 3; 
Params.lmin = 3;
Params.C = 4;
...
x0.sol = xsin.*ysin;
%subplot(1,2,1); 
surf(X,Y,x0.sol); xlim([0 1]); ylim([0 1]); pause;
...
fprintf('-------------------\n');
tmain_1 = tic;
[xF1,costTot] = VOpt_m(x0,f,g,delta,Params,update,surr);
time1 = toc(tmain_1);
fprintf('\n'); fprintf('Fevals: %.4e Fval: %.15e Time: %.2e \n',costTot,f(xF1.sol),time1);
if strcmp(problem,'p2d') || strcmp(problem,'tutorial') || strcmp(problem,'mins-lin');
...
% fprintf('---------------------\n');
% tmain_2 = tic;
% [xF2,costTot] = VOpt_m(x0,f,g,delta,Params,update,surr);
% time2 = toc(tmain_2);
...
% % Plot della soluzione
 ...
 surf(X,Y,xF1.sol); xlim(x_int); ylim(y_int);